...
class PackagingController extends ChangeNotifier {
  ...
  PackagingItem addItem(double sizeMl, int quantity) {
  ...
  void updateItem(String id, {double? sizeMl, int? quantity}) {
  ...
  bool deductBottle(double sizeMl) {
    final index = _items.indexWhere(
      (i) => (i.sizeMl - sizeMl).abs() < 0.001 && i.quantity > 0,
    );
  ...
  PackagingItem? bottleBySize(double sizeMl) {
    try {
      return _items.firstWhere((i) => (i.sizeMl - sizeMl).abs() < 0.001);
    } catch (_) {
      return null;
    }
  }
