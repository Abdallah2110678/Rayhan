...
class ReportExporter {
  Future<pw.Font> _loadReportFont() async {
    debugPrint('Using built-in Times font for PDF generation');
  ...
  Future<String> exportSalesReport({
    ...
    try {
      ...
      final arabicFont = await _loadReportFont();
      ...
      final totalSoldQuantity = sales.fold<double>(
        0,
          ...
          build: (context) => <pw.Widget>[
            ...
            pw.Wrap(
              ...
              children: <pw.Widget>[
                ...
                _summaryBox(
                  'Sold quantity',
                  formatMillimeters(totalSoldQuantity),
                ),
      ...
      try {
        ...
        debugPrint('Error writing PDF file: $error');
        rethrow;
      }
      return file.path;
  ...
  Future<File> _createReportFile({
    ...
    try {
      // Use app documents directory for reliable access
      ...
      final fileName = 'rayhan_report.pdf';
      return File('${reportsDirectory.path}${Platform.pathSeparator}$fileName');
    } catch (error) {
      debugPrint('Error creating report file: $error');
      rethrow;
    }
  }
}
